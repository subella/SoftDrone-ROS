...
class GraspStateMachine:
    ...
    def __init__(self, mission_manager):
        ...
        if not self._random_start:
            ...
            x_offset = .9
            y_offset = 0.6
            rand_theta = np.random.uniform(4/5. * np.pi, np.pi) 
            rand_r = np.random.uniform(1.0, 1.0)
            rand_z = np.random.uniform(1.0, 1.0)
    ...
    def _request_grasp_trajectory_once(self):
        if not self._waiting_on_grasp_trajectory_request and not self._already_requested:
            bool_msg = Bool()
            bool_msg.data = True
   ...
   #     self._target_omegas[2] = 0
    ...
    def _update_grasp_trajectory(self):
        ...
        trajectory_tracker = InterpTrajectoryTracker(polynomial, grasp_lengths, gripper_latency=gripper_latency, settle_time=self._trajectory_settle_time, alpha=1.0)
