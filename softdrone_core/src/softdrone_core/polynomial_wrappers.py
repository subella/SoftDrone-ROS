...
def multi_poly_trajectory_to_wrapper(msg):
    ...
    #speedup=1.
    #speedup = t_scale
    return MultiPolynomialWrapper(order, coeffs_x, coeffs_y, coeffs_z, t_list, speedup=speedup, t_start_global=t_start_global)


def evaluate_path_time(full_coeffs, T_list, t, speedup, order, der_order):
    T_list = list(T_list)
    cumsum = np.cumsum([0.0] + T_list + [np.inf])
    t = t*speedup
...
class MultiPolynomialWrapper:
    def __init__(self, order, coeffs_x, coeffs_y, coeffs_z, t_list, speedup, t_start_global=0):
        ...
        self.t_max = np.sum(t_list) / speedup
    ...
    def eval_rel_t(self, t, der=0, clip_to_tmax=True):
        ...
        x = evaluate_path_time(self.coeffs_x, self.t_list, t, self.speedup, self.order, der)
        y = evaluate_path_time(self.coeffs_y, self.t_list, t, self.speedup, self.order, der)
        z = evaluate_path_time(self.coeffs_z, self.t_list, t, self.speedup, self.order, der)
